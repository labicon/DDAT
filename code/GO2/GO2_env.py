...
Gymnasium Environment for the Unitree GO2
"""
...
class Go2Env(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, action_scale: float = 0.3,
        ...
        self.lowers = np.array([-0.7, -1.0, -2.2] * 4) # lower bound on the joint angles
        self.uppers = np.array([0.52, 2.1, -0.4] * 4) # upper bound on the joint angles
    ...
    def _reward_tracking_lin_vel(self):
        ...
        # lin_xy_vel = self.data.qvel[:2].copy()
        # lin_vel_error = np.sum(np.square(command[:2] - lin_xy_vel))
        lin_x_vel = self.data.qvel[0].copy()
        lin_vel_error = np.sum(np.square(command[0] - lin_x_vel))
        return 1.5*np.exp(-lin_vel_error)
    
    # def _reward_tracking_ang_vel(self):
    #     command = self.info['command']
    #     yaw_vel = self.data.qvel[5].copy()
